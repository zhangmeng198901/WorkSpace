var jdp = {
            ...
            $.fancybox.open({
                ...
                afterShow	: function(){
                    $("#upload-container").pluploadQueue({
                        ...
                        url : jdp.getContextPath()+'/file-system/attachment-quote/upload',
                        chunk_size: 0,
                        rename : true,
                        dragdrop: false,
                        autoSize : false,
                        ...
                        flash_swf_url : jdp.getContextPath()+'/scmarket-static/lib/jdp/plupload/Moxie.swf',
                        silverlight_xap_url : jdp.getContextPath()+'/scmarket-static/lib/jdp/plupload/Moxie.xap'
                    });
                },
                beforeClose	:function(){
                    if(callback && $.isFunction(callback)){
                        $.ajax({
                            url:jdp.getContextPath()+'/file-system/attachment-quote/queryFile/'+relationId,
                            type:'post',
    ...
    removeAttachmentQuote:function (id,fileObj,callback) {
        $.ajax({
            url:jdp.getContextPath()+"/file-system/attachment-quote/removeFile/"+id,
            data:{},
            type:"post",
            dataType:"json",
    ...
    assignMultiAccount:function (userEntityId,userType,detailType) {
        ...
        $.fancybox({
            type		: 'iframe',
            href:jdp.getContextPath()+'/admin/party/account/assignMulti?userEntityId='+getUserEntityId()
    ...
    assignOneAccount:function (userEntityId,userType,detailType) {
        ...
        $.fancybox({
            type: 'iframe',
            href:jdp.getContextPath()+'/admin/party/account/assignOne?userEntityId='+getUserEntityId()
            +'&userType='+getUserType()+'&detailType='+getDetailType(),
            fitToView :false,
            width : '90%',
            height : '90%',
...
function goDesktop() {
    location.href = jdp.getContextPath()+'/biz/desktop';
}
function goHome() {
    location.href = '/';
}
function goLoginPage() {
    location.href = jdp.getContextPath()+'/biz/login';
}
